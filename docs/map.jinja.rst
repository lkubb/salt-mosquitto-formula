...
We will see a quick setup to configure the ``mosquitto`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``mosquitto`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``mosquitto`` formulas under ``/srv/salt/mosquitto/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/mosquitto/parameters/dns:domain/
    ln -s dns:domain /srv/salt/mosquitto/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/mosquitto/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/mosquitto/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``mosquitto`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``mosquitto`` formulas under ``/srv/salt/mosquitto/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/mosquitto/parameters/roles
...
We create a configuration for the role ``mosquitto/server`` in ``/srv/salt/mosquitto/parameters/roles/mosquitto/server.yaml``:
...
We create another configuration for the role ``mosquitto/client`` in ``/srv/salt/mosquitto/parameters/roles/mosquitto/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles mosquitto/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles mosquitto/client
...
.. code-block:: console
    ...
    mv /srv/salt/mosquitto/parameters/dns:domain/  '/srv/salt/mosquitto/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-mosquitto-as-json:
      test.nop:
        - name: {{ mosquitto | json }}
...
Here is an example based on `template-formula/mosquitto/config/file.sls`_:
...
This ``sls`` file expose a ``mosquitto`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ mosquitto['winner'] }}
...
.. _template-formula/mosquitto/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/mosquitto/config/file.sls
