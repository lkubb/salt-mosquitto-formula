...
This installs the mosquitto composition,
manages the mosquitto configuration and then
starts the associated service.

``mosquitto.package``
^^^^^^^^^^^^^^^^^^^^^

This state will install the mosquitto composition only.

``mosquitto.config``
^^^^^^^^^^^^^^^^^^^^

This state will configure the mosquitto containers and has a dependency on ``mosquitto.install``
via include list.

``mosquitto.service``
^^^^^^^^^^^^^^^^^^^^^

This state will start the mosquitto container services and has a dependency on ``mosquitto.config``
via include list.
...
This state will remove the configuration of the mosquitto composition and has a
dependency on ``mosquitto.service.clean`` via include list.

``mosquitto.package.clean``
^^^^^^^^^^^^^^^^^^^^^^^^^^^

This state will remove the mosquitto composition and has a depency on
